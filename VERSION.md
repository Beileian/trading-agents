...
版本历史:
  v3.8.0 (2026-09-10): 收盘复盘双通道 P0 热修（全面评审蓝红蓝结论落地，托董 9/10）
    ...
    - 待办闭环（托董 9/10 15:56 决定）: 补传机制不做——IMA 失败时群摘要回指「次日盘前核验并手工补传」即为终态；测试残留文件已由托董在 IMA 客户端手工删除（回读确认盘后复盘目录仅剩 0909/0910 两份）。
  ...
  v3.7.1 (2026-09-10): 事件产业链分析 v1.2 — 输出注明信息来源出处（托董 9/10 指令）
    ...
    - 推送落款: 补 [版本号@commit] (此前该 cron 推送无落款, 违反铁律); git describe 命中 tag 时不重复带 hash。
    - 验证: 9/10 简报 --auto --dry-run 实跑, 来源行/推演行/落款均正常; 落盘 reports/event_chain_20260910.md。

  v3.7.0 (2026-09-10): A股收盘复盘「双通道」改造（评审整合结论 P0-1/2/3 + P1, 托董 9/10 授权, IMA路径=我的知识库/投资理财/盘后复盘）
    - 群聊改推「摘要」(≤500字): 一句话总判 + 指数 + 判定偏离(全对报N/M) + Rubrics标记 + 关注≤3。契约见 generate_closing_summary.py.
    - 全文不再直推群, 改入库 IMA「王海明的知识库→投资理财→盘后复盘」(folder_id=7503…), 新 ima_push_closing.sh 带读回校验.
    - P0-2 失败回指: 群摘要末行只在 IMA 回读成功才写「已存盘后复盘」, 否则写「暂存本地, 次日补传」.
    - P1 幂等: 当日已推标记文件避免重跑重复推送; 摘要生成失败才降级推全文.
    - 下游(paper_trading/style_rotation/update_daily_cache)消费的 close_snapshot/json 与原 md 路径时序不动 (P0-3).
  ...
  v3.5.6 (2026-09-07): 盘中预警源健康自愈机制（M3 红队评审确认，托董 9/07 授权）
    - 背景: 09-07 早盘腾讯行情源整体冻结 9:30 集合竞价快照(国睿科技冻在29.40连续10轮,真实跌至28.1),
      ...
      M3 红队(跨模型正交)普实根因: 源降级哨兵纯内存、开盘异动绕过1.5%一致门、JSONL记pre-dedup。
    - price_fetcher.py: _degraded_sources持久化到 price_freeze_state.json 顶层 degraded 键
      (cron每5分钟新进程实例级降级曾跨轮失忆,「整体冻结→降级30分钟」每轮打印但下轮T:on);
      窗口到期自动回血清哨兵落盘(自愈)。
    - price_watch.py: check_intraday_anomalies 接入 pp_map 源质量门——solo(unverified)/冻结/偏差>1.5%
      源不再触发即时异动(抑制并打[异动门🚫]日志); dedup key 含价位(防8/19中国长城同支撑48次刷屏);
      JSONL 改记实际推送列表+degraded正确标志(收盘复盘命中率不再系统性高估)。
    - 验证: py_compile全绿; 持久化跨进程实测(落盘→新实例读到→回血清除); P0-3 门三场景(solo/frozen/真跌)均正确抑制。
    - 盘前分析/推送两处 grep '方向[：:]' 改调 overseas signal_extract.py（修复 8/24~9/04 注入失败率≈50%，
      旧正则与 LLM 自由写作「方向判断：…｜…」不兼容）
    - 状态文件缺失时 Rubrics 不再默认满分 10，改标 n/a + WARN（防默认假满分掩盖状态断链）
    - event_chain_push.py main() 接入 push_to_group（此前 7/24 创建以来从未推送，静默无效运行 45 天；
      托董 9/06 拍板推群），支持 --dry-run，失败 exit 3
    - price_watch.py OPENCLAW_BIN 版本化硬编码(v22.22.0 已被 nvm 升级删除)→ which + nvm current 软链
  v3.5.2 (2026-08-11): 反幻觉校验方案C改造 — 失败不连锁（8/11 断供事故二次修复）
    - 问题陈述: 反幻觉校验重生成循环(最多2轮完整并发分析)成本高且收益不确定，set -e 下曾静默
      杀死脚本导致 trade_signals_ 断供；校验是质量门而非质量标注，阻塞主链路
    - 改造: 步骤1.5 改为单次校验+违规标注（去掉重生成循环，timeout 120 防 check hang）——
      trade_signals_ 生成不再依赖反幻觉校验通过；违规仅追加低质量标注供人工审阅
    - 配套: watchdog 盘前检查拆分为「分析产出 trade_signals_(06:35)」+「推送产物 premarket_(08:00)」
      两条，断供时告警直指环节（8/11 事故中单查 premarket_ 只能报"产出缺失"无法定位根因）
    - 验证: bash -n 语法通过；watchdog --json 实测精确报出分析/推送两条缺失
