...
def fetch_push2_via_vps(ticker: str) -> list[dict]:
    """通过VPS跳板机访问 push2 实时行情接口
    
    push2.eastmoney.com 对腾讯云IP做302→push2delay，但通过VPS跳板机
    可拿到完整实时数据（含成交量等更多字段）。
    作为 push2his 日K的补充兜底。
    """
    import symbols_config
    ...
    url = (
        ...
        f"?secid={secid}&fields=f43,f44,f45,f46,f47,f48,f60,f57,f58,f170"
    )
    
    try:
        r = subprocess.run([
            "curl", "-x", "http://49.51.33.96:8888", "-sL",
        ...
        print(f"  [WARN] {ticker} push2(VPS) 失败: {e}")
        return []
