...
输出要求：
{
  ...
  "仓位": 0-100,
  "行业背景": "简短的一句话（10字以内）",
  "具体风险": "1-2个具体风险点，必须含可观测触发条件，如'若跌破X元则止损'（60字以内）",
  "理由": "基于技术指标的客观理由，必须引用具体指标数值（60字以内）"
}

反幻觉铁律（违反任一即不合格）：
- 支撑位/阻力位必须基于MA或布林带给出具体价格，禁止'附近/平台/区域/一带'等模糊表述
- 趋势判断必须有具体技术指标数值支撑（均线价/RSI读数/MACD值）
- 同一标的的趋势判断与指标描述不得矛盾（如不能同时说'多头排列'又说'均线空头'）
- 具体风险必须写触发条件（'若跌破X则Y'），禁止'注意回调风险'式笼统表述
